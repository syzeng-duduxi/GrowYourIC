        ...
        def calc_vec(x, y, z, zeta):
            ...
            if x == 0 and y == 0:
                ...
                if x == 0:
                    ...
                    # v_z is solution of a quadratic equation, and v_x is
                    # calculated from v_z
                    polynome = [
                        y**2 / x**2 + 1,
                        2 * z * y / x**2 * v_z,
                        z**2 / x**2 * v_z**2 + v_z**2 - 1]
                    sol_v_y = np.roots(polynome)
                    v_y = sol_v_y[0]  # test what is the other root?
                    print(
                        "roots of the quadratic equation: {}. Root used: {}".format(
                            sol_v_y, v_y))
                    v_x = -y / x * v_y - z / x * v_z
            return v_x, v_y, v_z
        v_x, v_y, v_z = calc_vec(x, y, z, self.direction)
        # intersection between the trajectory and the rIC
        polynome = [v_x**2 + v_y**2 + v_z**2,
                    2 * (v_x * x + v_y * y + v_z * z),
                    x**2 + y**2 + z**2 - rIC**2]
        solutions = np.roots(polynome)  # 2 solutions, 1 for in, for out.
        # Here we have no difference between in and out, so we just say first
        # is in.
        self.in_point = CartesianPoint(
            x + solutions[0] * v_x,
            y + solutions[0] * v_y,
            z + solutions[0] * v_z)
        self.out_point = CartesianPoint(
            x + solutions[1] * v_x,
            y + solutions[1] * v_y,
            z + solutions[1] * v_z)
